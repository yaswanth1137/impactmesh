...
export class N8NConnector implements Connector<N8NWebhookPayload> {
  ...
  public normalize(
    ...
    const rawId = input.id || input.deal_id || input.execution_id || input.executionId;
    ...
    // 1. DEAL ACCEPTED
    if (normalizedEventType === 'deal.accepted' || normalizedEventType === 'deal_accepted') {
      const dealId = input.deal_id || input.id || rawId;
      const rawValue = input.value !== undefined
        ? input.value
        : (input.payload?.final_value ?? input.data?.final_value);
      const finalValue = requireNumber(rawValue, 'value');

      const decisionEvent: DecisionEvent<'deal_accepted'> = {
        id: `n8n-evt-${rawId}-${Date.now().toString(36)}`,
    ...
    if (normalizedEventType === 'capacity.changed' || normalizedEventType === 'capacity_changed') {
      ...
      const newCapacity = requireNumber(
      ...
      const decisionEvent: DecisionEvent<'capacity_changed'> = {
        ...
        payload: {
          ...
          new_capacity_hours: newCapacity,

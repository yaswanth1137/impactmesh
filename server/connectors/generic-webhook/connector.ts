...
export class GenericWebhookConnector implements Connector<GenericWebhookPayload> {
  ...
  public normalize(
    ...
    const rawExternalId = input.externalId ?? (input as Record<string, unknown>).external_id ?? input.id;
    if (rawExternalId === undefined || rawExternalId === null) {
      throw new NormalizationError('Generic webhook missing required field: externalId.', {
        code: 'MISSING_FIELD',
        connectorId: this.id,
        field: 'externalId',
      });
    ...
    if (externalId === '') {
      throw new NormalizationError('Generic webhook externalId cannot be empty.', {
        code: 'EMPTY_ENTITY_ID',
        connectorId: this.id,
        field: 'externalId',
        received: rawExternalId,
      });
    }

    if (input.occurredAt !== undefined && isNaN(Date.parse(String(input.occurredAt)))) {
      throw new NormalizationError(`Invalid date format for occurredAt: "${input.occurredAt}".`, {
        code: 'INVALID_DATE',
        connectorId: this.id,
        field: 'occurredAt',
    ...
    switch (canonicalType) {
      ...
      case 'feature_committed': {
        ...
        const featureCount = rawPayload.feature_count ?? rawPayload.featureCount;
        normalizedPayload = {
          feature_id: featureId,
          sprint_target: (rawPayload.sprint_target as string) || 'Sprint 42',
          committed_capacity_hours: hours,
          ...(featureCount !== undefined ? { feature_count: Number(featureCount) } : {}),
        };
        break;
      }
    ...
    const eventId = context?.deterministicId
      ? `gen-evt-${externalId.replace(/[^a-zA-Z0-9_-]/g, '_')}`
      : `gen-evt-${externalId}-${Date.now().toString(36)}`;
    ...
    const decisionEvent: DecisionEvent = {
      ...
      source_metadata: {
        ...
        actor: (input.actor || input.createdBy) as string | undefined,
